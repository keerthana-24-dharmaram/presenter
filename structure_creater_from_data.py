...
Now read these guidlines thoroughly and you will create the best presentation structure and the title for the presentation.
...
def create_presentation_structure_from_data(
    docs: List[Document], llm: LLM
) -> PresentationStructureWithTitle:
    print("\n> Creating presentation structure from existing data...\n")
    index = SummaryIndex(docs)
    query_engine = index.as_query_engine(
        llm=llm, output_cls=PresentationStructureWithTitle
    )
    return query_engine.query(TREE_SUMMARIZE_QUERY)
